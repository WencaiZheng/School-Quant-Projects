...
#        plot_tree(model,num_trees= 2).figure.set_size_inches(20, 20)#adjust the size of the picture
    ...
    for iMonth in monthTest:
        ...
        #-- pca
        if isPCA==1:
            dfTestX = pca.transform(dfTestX)#-返回绛维之后的数据
        #-- predict and get decision function
        if method == 'XGBOOST-C'  or method == 'XGBOOST-L' :
            #dfTestX.columns = names
            arPredictedCScore_csv = model.predict_proba(dfTestX)[:,1]#返回测试集中每个测试样例被分类为每个类的概率
...
#fac_1.to_excel(para.resultPath + '\\fac_1_year_factors_mt.xls',header=False,index=False)
